...
public class EqualSpaceLayout extends ViewGroup {
    ...
    public EqualSpaceLayout(Context context, AttributeSet attrs) {
        ...
        {
            TypedArray a = context.obtainStyledAttributes(attrs, new int[]{android.R.attr.layout_width,
                    android.R.attr.layout_height,});

            int layoutwidth = a.getLayoutDimension(0, LayoutParams.WRAP_CONTENT);
            int layoutheight = a.getLayoutDimension(1, LayoutParams.WRAP_CONTENT);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int maxChildWidth = mOrientation == LinearLayout.HORIZONTAL ? (MeasureSpec.getSize(widthMeasureSpec) / mNumVisibleChildren)
                : MeasureSpec.getSize(widthMeasureSpec);
        int maxChildHeight = mOrientation == LinearLayout.VERTICAL ? (MeasureSpec.getSize(heightMeasureSpec) / mNumVisibleChildren)
                : MeasureSpec.getSize(heightMeasureSpec);

        final int childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(MeasureSpec.getSize(maxChildWidth),
                MeasureSpec.AT_MOST);
        final int childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(MeasureSpec.getSize(maxChildHeight),
                MeasureSpec.AT_MOST);

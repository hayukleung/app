...
public class CollapsibleView extends RecyclerView {
    ...
    public CollapsibleView(Context context, @Nullable AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            // 自定义属性
            int defStyleRes = 0;
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CollapsibleView, defStyle, defStyleRes);
            mBgResIdOrg = a.getResourceId(R.styleable.CollapsibleView_drawableOrg, R.drawable.selector_tree_org);
            mBgResIdUsr = a.getResourceId(R.styleable.CollapsibleView_drawableUsr, R.drawable.selector_tree_usr);
            mBgResIdClosed = a.getResourceId(R.styleable.CollapsibleView_drawableClosed, R.drawable.ic_expand_more_white_24dp);
            mBgResIdOpened = a.getResourceId(R.styleable.CollapsibleView_drawableOpened, R.drawable.ic_expand_less_white_24dp);
            a.recycle();
        }
    ...
    public void setAdapter(Adapter adapter) {
        throw new UnsupportedOperationException("Use setAllElements(allElements).setVisibleElements(visibleElements).setOnCollapsibleClickListener(onCollapsibleClickListener).commit() instead of this.");
    }
    ...
    public CollapsibleView setOnCollapsibleClickListener(OnCollapsibleClickListener onCollapsibleClickListener) {
        this.mCollapsibleClickListener = onCollapsibleClickListener;
        return this;
    }

    public void commit() {
        mCollapsibleAdapter = new CollapsibleAdapter(mContext, mBgResIdOrg, mBgResIdUsr, mBgResIdClosed, mBgResIdOpened, mAllElements, mVisibleElements, mCollapsibleClickListener);

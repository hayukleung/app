...
public class DiskBasedCache implements Cache {
    ...
    public synchronized Entry get(String key) {
        ...
        try {
            ...
            Ln.d("%s: %s", file.getAbsolutePath(), e.toString());
            remove(key);
            return null;
        } finally {
            if (cis != null) {
    ...
    public synchronized void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                Ln.e("Unable to create cache dir %s", mRootDirectory.getAbsolutePath());
            }
            return;
        }
    ...
    /*
     * Homebrewed simple serialization system used for reading and writing cache headers on disk. Once upon a time, this used the standard Java Object{Input,Output}Stream, but the default
     * implementation relies heavily on reflection (even for standard types) and generates a ton of garbage.
     */
    ...
    public synchronized void put(String key, Entry entry) {
        ...
        if (!deleted) {
            Ln.d("Could not clean up file %s", file.getAbsolutePath());
        }
    }
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            Ln.d("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
        }
    }
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        Ln.v("Pruning old cache entries.");
        ...
        while (iterator.hasNext()) {
            ...
            if (deleted) {
                ...
                Ln.d("Could not delete cache entry for key=%s, filename=%s", e.key, getFilenameForKey(e.key));
            }
            iterator.remove();
            prunedFiles++;
        ...
        Ln.v("pruned %d files, %d bytes, %d ms", prunedFiles, (mTotalSize - before), SystemClock.elapsedRealtime() - startTime);
    }

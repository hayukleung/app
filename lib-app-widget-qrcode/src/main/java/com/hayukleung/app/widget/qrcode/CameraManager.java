...
public final class CameraManager {
     ...
     * This target helps with alignment as well as forces the user to hold the device far enough away to ensure the image will be in focus.
     *
    ...
    public Rect getFramingRect() {
        ...
        if (framingRect == null) {
            ...
            int leftOffset = (screenResolution.x - minSize) / 2;
            int topOffset = (screenResolution.y - minSize) / 3;

            // framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);

            framingRect = new Rect(leftOffset, topOffset, leftOffset + minSize, topOffset + minSize);
    ...
    public Rect getFramingRectInPreview() {
        if (framingRectInPreview == null) {
            ...
            // int width;
            // int height;
            // int minSize = Math.min(cameraResolution.x, cameraResolution.y);
            // width = height = minSize * 3 / 5;
     ...
     */
    /*
     * public Point[] convertResultPoints(ResultPoint[] points) { Rect frame = getFramingRectInPreview(); int count = points.length; Point[] output = new Point[count]; for (int x = 0; x < count; x++) { output[x] = new Point(); output[x].x = frame.left + (int) (points[x].getX() + 0.5f); output[x].y = frame.top + (int) (points[x].getY() + 0.5f); } return output; }
     */
